    ...
    while (queue.length > 0) {
        ...
        for (const n of neighbors) {
            if (n.x >= 0 && n.x < gridWidth && n.y >= 0 && n.y < gridHeight) {
                const key = `${n.x},${n.y}`;
                if (!visited.has(key)) {
                    const cellType = occupied[n.y][n.x];
                    if (cellType === 'weg' || cellType === 'rathaus') {
                        queue.push(n);
                        visited.add(key);
                    }
                }
...
// Jede Sekunde: prüfen, aktualisieren
setInterval(() => {
    // Einwohner neu berechnen
    let newPopulation = 0;
    let newFreePopulation = 0;
    houses.forEach(house => {
        const connected = isConnected(house.x, house.y);
    ...
    goldmines.forEach(mine => {
        const connected = isConnected(mine.x, mine.y);
