...
function bfsCheck(xTarget, yTarget) {
    ...
    while (queue.length > 0) {
        ...
        if ((currX >= xTarget && currX <= xTarget + 1) && (currY >= yTarget && currY <= yTarget + 1)) {
            return true;
        }
        ...
        for (const n of neighbors) {
            if (n.x >= 0 && n.x < gridWidth && n.y >= 0 && n.y < gridHeight) {
                const nType = occupied[n.y][n.x];
                if ((nType === 'weg' || nType === 'rathaus') && !visited.has(`${n.x},${n.y}`)) {
                    queue.push(n);
                }
            }
        }
...
setInterval(() => {
    houses.forEach(house => {
        const isNowConnected = bfsCheck(house.x, house.y);
        if (isNowConnected && !house.connected) {
            population += house.popGain;
            freePopulation += house.popGain;
            house.connected = true;
        }
        setBuildingColor(house.x, house.y, isNowConnected);
    });

    goldmines.forEach(mine => {
        const isNowConnected = bfsCheck(mine.x, mine.y);
        mine.connected = isNowConnected;
        if (isNowConnected) {
            gold += mine.level * 5;
        }
        setBuildingColor(mine.x, mine.y, isNowConnected);
    });
    ...
    for (let dy = 0; dy < 2; dy++) {
        for (let dx = 0; dx < 2; dx++) {
            ...
            if (!connected) {
                ...
                if (occupied[y + dy][x + dx] === 'haus') {
                    cell.style.backgroundColor = 'lightblue';
                } else if (occupied[y + dy][x + dx] === 'goldmine') {
