...
let selectedMine = null; // aktuell geklickte Mine für Upgrade
...
function openUpgradePopup(x, y) {
    ...
    document.getElementById('popup-text').textContent = `Goldmine Level ${selectedMine.level} verbessern für ${selectedMine.level * 100} Gold?`;
...
function confirmUpgrade() {
    if (!selectedMine) return;
    const upgradeCost = selectedMine.level * 100;
    if (gold >= upgradeCost) {
        gold -= upgradeCost;
        selectedMine.level++;
        alert('Upgrade erfolgreich! Level: ' + selectedMine.level);
    } else {
        alert('Nicht genug Gold!');
    }
    closePopup();
...
// Richtige BFS für Verbindung prüfen
