...
function placeBuilding(x, y) {
    ...
    if (selectedBuilding === 'goldmine' && freePopulation < 5) {
        alert('Nicht genug freie Einwohner!');
        return;
    }
    ...
    if (buildingType === 'goldmine') {
        goldmines.push({ x: x, y: y, level: 1, connected: false });
        addUpgradeButton(x, y);
    }

    if (buildingType === 'haus') {
        houses.push({ x: x, y: y, popGain: 5, connected: false });
    }
...
function findConnectedPositions() {
    const visited = new Set();
    const queue = [{ x: rathausPosition.x, y: rathausPosition.y }];
    visited.add(`${rathausPosition.x},${rathausPosition.y}`);
...
function isBuildingConnected(x, y, visitedPositions) {
...
function setBuildingColor(x, y, connected) {
    for (let dy = 0; dy < 2; dy++) {
        for (let dx = 0; dx < 2; dx++) {
            ...
            if (connected) {
                ...
                cell.style.filter = "brightness(60%)";
            }
        }
    }
}
...
// Jede Sekunde prüfen
setInterval(() => {
    const visitedPositions = findConnectedPositions();

    let newPopulation = 0;
    let newFreePopulation = 0;

    houses.forEach(house => {
        const connected = isBuildingConnected(house.x, house.y, visitedPositions);
        house.connected = connected;
        if (connected) {
            newPopulation += house.popGain;
            newFreePopulation += house.popGain;
        }
        setBuildingColor(house.x, house.y, connected);
    });

    population = newPopulation;
    freePopulation = newFreePopulation;

    goldmines.forEach(mine => {
        const connected = isBuildingConnected(mine.x, mine.y, visitedPositions);
        mine.connected = connected;
        if (connected) {
            gold += mine.level * 5;
        }
        setBuildingColor(mine.x, mine.y, connected);
    });
