...
function placeBuilding(x, y) {
    ...
    if (selectedBuilding === 'haus') {
        size = { width: 2, height: 2 };  // Häuser nehmen 2x2 Felder ein
        cost = 100;
        buildingType = 'haus';
        popChange = 5;
    } else if (selectedBuilding === 'goldmine') {
        size = { width: 2, height: 2 };  // Goldminen nehmen auch 2x2 Felder ein
        cost = 150;
...
// Häuserproduktion (alle 1 Sekunde)
setInterval(() => {
    const houses = [];
    for (let y = 0; y < gridHeight; y++) {
        for (let x = 0; x < gridWidth; x++) {
            if (occupied[y][x] === 'haus') {
                houses.push({ x: x, y: y });
            }
        }
    }
    ...
    houses.forEach(house => {
        if (isConnected(house.x, house.y)) {
            freePopulation += 1; // Beispiel: Erhöht die freien Einwohner
        }
    });
    updateResources();
}, 1000); // Alle 1 Sekunde
