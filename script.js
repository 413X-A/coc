...
function placeBuilding(x, y) {
    ...
    if (buildingType === 'goldmine') {
        goldmines.push({ x, y, level: 1 });
        addUpgradeButton(x, y);
        freePopulation -= 5; // 5 Arbeiter werden sofort für die Mine verbraucht
    }

    if (buildingType === 'haus') {
        // erst später Einwohner hinzufügen, wenn verbunden
        pendingHouses.push({ x, y, popGain });
    }
...
setInterval(() => {
    ...
    // Häuser-Check
    for (let i = pendingHouses.length - 1; i >= 0; i--) {
        const house = pendingHouses[i];
        if (isConnected(house.x, house.y)) {
            population += house.popGain;
            freePopulation += house.popGain;
            pendingHouses.splice(i, 1); // Haus ist fertig verbunden -> entfernen
        }
    }
